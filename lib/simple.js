...
const {
	WAConnection
} = require('@adiwajshing/baileys/lib/WAConnection/0.Base')
...
`.trim()
			...
			return await this.sendMessage(jid, vcard, MessageType.contactsArray, {
				quoted,
				...options
			})
		}
		...
		async prepareMessageMedia(buffer, mediaType, options = {}) {
			...
			if (requiresDurationComputation) {
				try {
					...
					this.logger.debug({
						error
					}, 'failed to obtain audio duration: ' + error.message)
				}
			}
		...
		async sendVideo(jid, url, caption, quoted, opt) {
			await download(url, 'mp4', async ({
				...
				return await this.sendMessage(jid, video, MessageType.video, {
					caption: caption,
					quoted,
					...opt
				})
		...
		async sendButtonsVid(jid, text, path, buttons = [], footer = "wuis", quoted, options) {
			...
			return this.sendMessage(jid, btnMsg, MessageType.buttonsMessage, {
				quoted,
				...options
			})
		}

		async sendButtonsGif(jid, text, path, buttons = [], footer = "wuiss", quoted, options) {
			let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await getBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
			let vid = await this.prepareMessage(jid, buff, MessageType.video, {
				mimetype: 'video/gif '
			})
		...
		async sendFile(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
			...
			if (options.asSticker) mtype = MessageType.sticker
				...
				if (/webp/.test(type.mime) && file.length <= 1 << 20) mtype = MessageType.sticker
					...
					try {
						return await this.sendVideo(jid, file, caption, quoted, {
							...opt,
							...options
						})
					} catch (e) {
			...
			return await this.sendMessage(jid, file, mtype, {
				...opt,
				...options
			})
		}
		...
		async sendSticker(jid, media, quoted, opts = {}) {
			...
			return await this.sendMessage(jid, await convert(medias.data, null, opts.packname || global.packname, opts.author || global.author), 'stickerMessage', {
				quoted: quoted,
				...opts
			})
		}
		...
		reply(jid, text, quoted, options) {
			return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, {
				contextInfo: {
					...
					externalAdReply: {
						...
						body: '© Anti',
						sourceUrl: '',
						thumbnail: "https://i.ibb.co/PMLygL5/IMG-20211127-WA0009.jpg"
					}
				},
				sendEphemeral: true,
				quoted,

...
handler.all = async function(m, {
	...
	if (/^.*tiktok/i.test(m.text)) {
		tiktok(m.text).then(async res => {
			...
			await this.sendVideo(m.chat, json.nowm, '*KYAAA*', m, {
				thumbnail: buf
			})
		}).catch(_ => _)
	}
	...
	if (/^.*cocofun/i.test(m.text)) {
		let res = await fetch(global.API('jojo', '/api/cocofun-no-wm', {
			url: m.text
		}))
		if (!res.ok) throw await res.text()
		let json = await res.json()
		await this.sendVideo(m.chat, json.download, `*KYAAA*`, m, {
			thumbnail: buf
		})
	}
	...
	if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
		pin(m.text).then(async res => {
			...
			await this.sendVideo(m.chat, json.data.url, `*KYAAA*`, m, {
				thumbnail: buf
			})
		}).catch(_ => _)
	}
	...
	if (/^.*twitter.com\//i.test(m.text)) {
		twitter(m.text).then(async res => {
			...
			for (let {
				...
				this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), `*© stikerin*`, m, false, {
					thumbnail: buf
				})
			}
		}).catch(_ => _)
	...
	if (/^https?:\/\/.*youtu/i.test(m.text)) {
		...
		let {
			...
			filesize,
			filesizeF
		} = await yta(m.text, servers.includes(server) ? server : servers[0])

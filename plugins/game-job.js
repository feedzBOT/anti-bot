let handler = async (m, {
	...
	if (!args[0] || !args[1]) {
		throw `*[ JOB ERROR ]*\n\nMasukkan nama profesinya dan harga jasa. Misal :\n${usedPrefix}job maling 100000\n\nPekerjaan yang tersedia : pijat, lonte, sepong, maling`
	} else {
		if (parseInt(args[1]) < 1000 || parseInt(args[1]) > 300000) {
			throw '*[ JOB ERROR ]*\n\nHarga jasa minimal Rp1.000 & maksimal Rp300.000'
		}
		...
		if (args[0] === "lonte") {
			...
			throw '*[ JOB ERROR ]*\n\nJenis pekerjaan yang tersedia adalah maling, lonte, sepong, pijat'
		}
	}
}

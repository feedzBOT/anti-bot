	...
	try {
		...
		if (global.db.data.users[m.sender].healt > 79) {
			if (new Date - global.db.data.users[m.sender].lastadventure > 300000) {
				...
				let str = `
Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['🇯🇵Jepang', '🇰🇷Korea', '🏖️Bali', '🇺🇸Amerika', '🇮🇷Iraq', '🇸🇦Arab', '🇵🇰Pakistan', '🇩🇪German', '🇫🇮Finlandia', '💤Ke bawa dunia mimpi', '🌎Ujung dunia', '🗿Mars', '🌕Bulan', '🪐Pluto', '☀️Matahari', '🥰Hatinya dia', '...'])} dan mendapatkan
*🎟️exp:* ${exp} 
*💸uang:* ${uang}
*🗑️sampah:* ${sampah}${potion == 0 ? '' : '\n*🍾Potion:* ' + potion + ''}${diamond == 0 ? '' : '\n*💎diamond:* ' + diamond + ''}${common == 0 ? '' : '\n*🗃️common crate:* ' + common + ''}${uncommon == 0 ? '' : '\n*🗃️uncommon crate:* ' + uncommon + ''}
`.trim()
				conn.reply(m.chat, str, m)
				if (mythic > 0) {
					global.db.data.users[m.sender].mythic += mythic * 1
					conn.reply(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + ' 🗃️Mythic Crate', m)
				}
				if (legendary > 0) {
